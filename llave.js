...
(function _setupLoginUI() {
    ...
    function _initSamuraiArena() {
        ...
        let isAttacking = false;
            ...
            "📱 Disfruta en cualquier pantalla",
            "🎉 Renovación súper rápida",
            "🎁 Pregunta por nuestras ofertas",
            "💸 Los mejores precios del mercado",
            "✅ Activación inmediata",
            "🦸‍♂️ Catálogo de súper héroes completo",
            "🌟 Series exclusivas y estrenos"
        ];
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (t && u) {
        // âœ… FIX: usar _ocultarLogin() en lugar de .style.display = 'none'
...
window.initThreeJSPosters = async function(bannerElement) {
    ...
    const posterSize = { h: 40, w: 27, padding: 2, cols: 15, rows: 10, resIndex: 2 };
    ...
    btnPerspective.innerHTML = '<span class="material-icons-round">view_in_ar</span>';
    
    let currentMode = 0; // 0: Normal, 1: Abajo, 2: Arriba
    btnPerspective.addEventListener('click', () => {
        currentMode = (currentMode + 1) % 3;
        btnPerspective.classList.add('spinning');
        setTimeout(() => btnPerspective.classList.remove('spinning'), 400);
    ...
    bannerElement.appendChild(btnPerspective);
    ...
    // --- RAYCASTER PARA CLICS EN IMÁGENES ---
    const raycaster = new THREE.Raycaster();
    const mouse = new THREE.Vector2();

    renderer.domElement.addEventListener('click', (event) => {
        if (!disableAnimate) return; // Solo funciona en pausa
        ...
        const intersects = raycaster.intersectObjects(assetGroup.children, true);

        if (intersects.length > 0) {
            const object = intersects[0].object;
            if (object.userData && object.userData.name) {
                const nombrePelicula = object.userData.name;
                
                // Copiar al portapapeles
                navigator.clipboard.writeText(nombrePelicula).then(() => {
                    Swal.fire({
                    ...
                    // Mostrar alerta de todas formas si falla el copiado
                    Swal.fire({
                        title: nombrePelicula,
                        icon: 'info',
                        toast: true,
    ...
    function renderPostersFast() {
        ...
        for (let i = 0; i < totalPosters; i++) {
            ...
            const imgIndex = Math.floor(Math.random() * posterData.length);
            const url = posterData[imgIndex].url;
            ...
            poster.userData = { name: posterData[imgIndex].name };
            poster.position.x = x;
            rowGroup.add(poster);
        }
    }
